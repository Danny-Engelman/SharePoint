...
############################################## Inline change item Status
="<script>"
&"function ICCupdateselection( me ){"
&"  var TRnode, clientContext, itemid, list, item, value;"
&"  TRnode=me.parentNode.parentNode.parentNode;"
...
&"		var value=item.get_item(""Status"");"
&"		item.set_item(""Status"",me.value);"
&"		item.update();"
&"		clientContext.load(item);"
&"		clientContext.executeQueryAsync("
&"			(function(){me.parentNode.parentNode.style.backgroundColor=""LimeGreen""}) ,"
&"			(function(){alert(""failed ICC function"")})"
&"		)})"
&"}"
&"var S='"&Status&"';"
&"var H='<SELECT onchange=""javascript:ICCupdateselection(this)"">';"
&"var O=['Not Started','In Progress','Completed','Deferred','Waiting on someone else'];"
&"for(i=0;i<O.length;i++){H=H+'<OPTION value=""'+O[i]+'"" '+(S==O[i]?'Selected':'')+'>'+O[i]+'</OPTION>';}"
&"H=H+'</SELECT>';"
&"document.write(H);"
&"</script>"
